 ...
 */
public class MtStHelensExp_913327304_912684174 implements AIModule
{
	private double getHeuristic(final TerrainMap map, final Point pt1, final Point pt2){
		double multiplier = 0.0;
        double heuristic = 0.0;

		if (map.getTile(pt2) - map.getTile(pt1) > 0)
			multiplier = 2;
		 else if (map.getTile(pt2) - map.getTile(pt1) < 0)
			multiplier = 0.5;
		 else
			multiplier = 1;
        
        double totalDistance = (Math.abs(pt2.x-pt1.x)+Math.abs(pt2.y-pt1.y))-Math.min(Math.abs(pt2.x-pt1.x),Math.abs(pt2.y-pt1.y));
        double dHeight = Math.abs(map.getTile(pt2) - map.getTile(pt1));
        heuristic = multiplier*dHeight;
        totalDistance -= dHeight;
